...
SpatialHash::SpatialHash(float spacing, int table_size) : spacing(spacing), table_size(table_size) {
...
void SpatialHash::update(const std::vector<Vector2>& predicted_positions) {
    std::fill(start_indices.begin(), start_indices.end(), -1);

    num_particles = predicted_positions.size();
    ...
    for (size_t i = 0; i < num_particles; i++) {
        int cell_x = (int)std::floor(predicted_positions[i].x / spacing);
        int cell_y = (int)std::floor(predicted_positions[i].y / spacing);
    ...
    std::sort(spatial_lookup.begin(), spatial_lookup.begin() + num_particles,
              [](const spatial_entry& a, const spatial_entry& b) { return a.cell_key < b.cell_key; });
